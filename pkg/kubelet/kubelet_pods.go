...
func (kl *Kubelet) PodResourcesAreReclaimed(pod *v1.Pod, status v1.PodStatus) bool {
	...
	if kl.podVolumesExist(pod.UID) && !kl.basicInfo.KeepTerminatedPodVolumes {
...
func (kl *Kubelet) generateAPIPodStatus(pod *v1.Pod, podStatus *kubecontainer.PodStatus) v1.PodStatus {
	...
	if kl.kubeClient != nil {
		hostIP, err := kl.basicInfo.GetHostIPAnyWay()
		if err != nil {
...
func (kl *Kubelet) cleanupOrphanedPodCgroups(cgroupPods map[types.UID]cm.CgroupName, activePods []*v1.Pod) {
	...
	for uid, val := range cgroupPods {
		...
		if podVolumesExist := kl.podVolumesExist(uid); podVolumesExist && !kl.basicInfo.KeepTerminatedPodVolumes {
